...
Metric state is session-local for this implementation. The existing camera calibration persistence code is not repurposed to persist metric state. A new process starts `UNCALIBRATED`; no persisted metric matrix is trusted or loaded. The metric record is one shared object, not one record per camera. `GET /calibration/status` also exposes an aggregate `calibration` stage: `UNCALIBRATED`, `UNVERIFIED`, `CALIBRATED`, `METRIC_CALIBRATED` or `STALE`, plus the evaluated `metric_calibration` state and the unchanged per-camera statuses. `METRIC_CALIBRATED` means the shared metric transform is currently usable; it does not relabel unrelated camera records.

## Metric thresholds and applicability

Configuration has a separate validated metric threshold group. The initial defaults are `ransac_reprojection_threshold_mm = 3.0`, `max_capture_corner_jitter_pixels = 2.0`, `max_mean_fit_error_mm = 2.0`, `max_fit_error_mm = 5.0`, `min_inlier_ratio = 0.5`, `min_unique_target_fiducials = 4`, `min_capture_marker_ratio = 0.8` and `min_spatial_coverage = 0.5`; they remain configuration rather than scattered constants. Thresholds are in physical units unless explicitly named otherwise:
