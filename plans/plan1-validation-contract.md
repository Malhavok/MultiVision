...
- Camera calibration and verification use a consecutive settled-frame burst. Complete common tags must cover at least half the median per-frame tag count, with the existing two-tag minimum. Per-corner medians reduce isolated detector noise; robust MAD-derived temporal sigma is recorded for the median, 95th percentile and maximum corner jitter. A configured p95 sigma ceiling rejects unstable bursts before tolerance derivation.
...
- Persisted calibration stores stable camera ID, camera resolution, projector resolution, calibration version, both matrices, metrics and timestamp. Loaded data starts `UNVERIFIED`; verification against known fiducials is required before use. Verification derives reprojection limits from stored fit residuals plus three measured noise sigmas rather than relying on a universal pixel limit. Verification below configured thresholds gives `CALIBRATED`; failure gives `STALE`.
